...
export default function RestaurantLayout() {
  ...
  /* ★ null = لود نشده / [] = واقعاً خالی / [...] = سرویس‌های فعال */
  const [enabledServices, setEnabledServices] = useState(null);
  const [servicesLoaded, setServicesLoaded] = useState(false);

  /* ★ slug از useParams یا از URL */
  const slug = useMemo(() => {
  ...
  return (
    <Box dir={isRtl ? "rtl" : "ltr"} sx={{ minHeight: "100vh", bgcolor: C.bg, position: "relative", fontFamily: "'Vazirmatn', 'Plus Jakarta Sans', sans-serif", overflowX: "hidden" }}>
      ...
      {isMobile && (
        <Drawer anchor={isRtl ? "right" : "left"} open={sidebarOpen} onClose={() => setSidebarOpen(false)} slotProps={{ paper: { sx: { width: 280, bgcolor: "transparent", border: "none" } } }}>
          {sidebarContent}
        </Drawer>
      )}
