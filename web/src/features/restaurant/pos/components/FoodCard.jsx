...
export default function FoodCard({
  ...
  useEffect(() => {
    ...
    setStock(food.stock != null ? food.stock : "");
    initialStock.current = food.stock != null ? food.stock : "";
    setImageUrl(food.image || null);
  }, [food.id, food.price, food.stock, food.image]);

  const serverDiscount =
    food.price && food.final_price && food.final_price < food.price
      ? food.price - food.final_price
      : 0;

  const serverDiscountPercent =
    price > 0 ? Math.round((serverDiscount / price) * 100) : 0;
  const isCleanPercent =
    serverDiscountPercent > 0 &&
    Math.round((price * serverDiscountPercent) / 100) === serverDiscount;
  ...
  const catDiscountAmount =
    categoryDiscountType === "percent"
      ? Math.round((price * categoryDiscount) / 100)
      : Math.min(categoryDiscount, price);

  const displayDiscount = editMode ? userDiscountAmount : serverDiscount;
  const totalDiscount = Math.min(displayDiscount, price);
  const basePrice =
    food.final_price != null && food.final_price < food.price
      ? food.final_price
      : price;

  const effectivePrice = editMode
    ? Math.max(
        0,
        price - Math.min(userDiscountAmount + catDiscountAmount, price),
      )
    : Math.max(0, basePrice - Math.min(catDiscountAmount, basePrice));

  const hasServerDiscount =
    food.final_price != null && food.final_price < food.price;
  const hasCatDiscount = categoryDiscount > 0;
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      await client.post(`/api/dictionary/food/${food.id}/update/`, payload);
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      const res = await client.post(
        `/api/dictionary/food/${food.id}/update/`,
        fd,
        {
  ...
  const showDiscountBadge =
    hasServerDiscount || hasCatDiscount || (editMode && discount > 0);
  ...
  return (
    ...
    >
      ...
      >
        {isPinned ? (
          <PushPin
            sx={{ fontSize: { xs: 14, sm: 16 }, transform: "rotate(45deg)" }}
          />
        ) : (
        ...
        >
          {editMode
            ? discount > 0
              ? discountMode === "percent"
                ? `${discount}%`
                : `${formatNum(userDiscountAmount)} ت`
              : ""
            : (() => {
                const parts = [];
      ...
      >
        {imageUrl ? (
          ...
          <Typography sx={{ fontSize: { xs: 36, sm: 48 }, opacity: 0.25 }}>
            🍽️
          </Typography>
        )}
        {editMode && (
        ...
        {editMode ? (
          ...
          >
            ...
            <Box>
              ...
              <input
                type="text"
                value={price ? formatNum(price) : ""}
                onChange={(e) => {
                  const raw = e.target.value.replace(/,/g, "");
                  if (/^\d*$/.test(raw)) setPrice(raw === "" ? 0 : Number(raw));
                }}
                placeholder={isRtl ? "مبلغ رو وارد کنین" : "Enter price"}
                style={inputSx}
              />
            </Box>
            ...
            <Box>
              ...
              <input
                type="text"
                value={discount ? formatNum(discount) : ""}
                onChange={(e) => {
                  const raw = e.target.value.replace(/,/g, "");
                  if (/^\d*$/.test(raw)) {
                    const val = raw === "" ? 0 : Number(raw);
                    setDiscount(
                      discountMode === "percent" ? Math.min(100, val) : val,
                    );
                  }
                }}
                placeholder={
              ...
              {discount > 0 && (
                ...
                >
                  {isRtl
                    ? `قیمت نهایی: ${formatNum(effectivePrice)} تومان`
                    : `Final price: ${formatNum(effectivePrice)} Toman`}
                </Typography>
              )}
              {hasServerDiscount && (
                <Typography
                ...
                >
                  {isCleanPercent
                    ? isRtl
                      ? `تخفیف فعلی: ${serverDiscountPercent}%`
                      : `Current: ${serverDiscountPercent}%`
                    : isRtl
                      ? `تخفیف فعلی: ${formatNum(serverDiscount)} تومان`
                      : `Current: ${formatNum(serverDiscount)} Toman`}
                </Typography>
              )}
              {hasCatDiscount && (
                <Typography
                ...
                >
                  {categoryDiscountType === "percent"
                    ? isRtl
                      ? `تخفیف دسته: ${categoryDiscount}%`
                      : `Category: ${categoryDiscount}%`
                    : isRtl
                      ? `تخفیف دسته: ${formatNum(catDiscountAmount)} تومان`
                      : `Category: ${formatNum(catDiscountAmount)} Toman`}
                </Typography>
              )}
            </Box>
            ...
            <Box>
              ...
              <Box sx={{ display: "flex", gap: 0.6, alignItems: "center" }}>
                <input
                  type="text"
                  value={stock !== "" && stock != null ? formatNum(stock) : ""}
                  onChange={(e) => {
                    const raw = e.target.value.replace(/,/g, "");
                    if (/^\d*$/.test(raw))
                      setStock(raw === "" ? "" : Math.max(0, Number(raw)));
                  }}
                  placeholder={isRtl ? "تعداد رو وارد کنین" : "Enter quantity"}
                  style={{ ...inputSx, flex: 1 }}
                />
                <Box
                  onClick={() =>
                    setStock((s) => (s === "" ? 10 : Number(s) + 10))
                  }
                  sx={{
                    height: 34,
                    px: 1.2,
