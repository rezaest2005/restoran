...
export default function FoodCard({
  ...
  const initialStock = useRef(food.stock ?? 10);
  const fileRef = useRef(null);
  const outOfStock = stock <= 0;
  const lowStock = stock > 0 && stock <= 5;

  // ★ همگام‌سازی وقتی food از سرور آپدیت میشه (بدون setState در render)
  useEffect(() => {
    setPrice(food.final_price || food.price || 0);
    setStock(food.stock ?? 10);
    initialStock.current = food.stock ?? 10;
    setImageUrl(food.image || null);
  }, [food.id, food.final_price, food.price, food.stock, food.image]);
  ...
  const inputSx = {
    ...
    border: `1.5px solid ${C.olive}22`, background: `${C.olive}08`,
  ...
  // ★ ذخیره تغییرات به سرور
const handleSave = async (e) => {
  e.stopPropagation();
  setSaving(true);
  try {
    const client = (await import("../../api/client")).default;
    await client.post(`/api/dictionary/${food.id}/update/`, {
      price: price,
      final_price: price,
      stock: stock,
    });
  ...
  const dec = (e) => {
    ...
    setQty(q => {
      if (q > 0) {
        setStock(s => Math.min(initialStock.current, s + 1));
  ...
  return (
    <Box
      ...
      sx={{
        ...
        minHeight: { xs: 120, sm: 140 }, userSelect: "none",
    ...
    >
      ...
      <IconButton
        ...
        sx={{
          ...
          zIndex: 5, width: { xs: 26, sm: 28 }, height: { xs: 26, sm: 28 },
          borderRadius: "7px",
          bgcolor: isPinned ? C.gold : "rgba(0,0,0,0.35)",
          color: isPinned ? "#fff" : "rgba(255,255,255,0.8)",
          opacity: hovered || isPinned ? 1 : 0.5,
          border: isPinned ? `2px solid ${C.gold}` : "2px solid rgba(255,255,255,0.15)",
          transition: "all 0.2s ease",
          "&:hover": { bgcolor: isPinned ? C.gold : "rgba(0,0,0,0.5)", transform: "scale(1.1)", opacity: 1 },
        }}
      >
        {isPinned
      ...
      {discountAmount > 0 && !editMode && (
        <Box sx={{
          position: "absolute", top: 6,
          left: isRtl ? 40 : "auto", right: isRtl ? "auto" : 40,
          zIndex: 3, bgcolor: C.burgundy, color: "#fff",
          fontSize: { xs: 10, sm: 11 }, fontWeight: 800,
      ...
      {outOfStock && !editMode && (
        <Box sx={{
          position: "absolute", top: 6,
          right: isRtl ? "auto" : 6, left: isRtl ? 6 : "auto",
          zIndex: 3, bgcolor: C.danger, color: "#fff",
          fontSize: { xs: 10, sm: 11 }, fontWeight: 800,
          px: 0.8, py: 0.15, borderRadius: "6px",
          fontFamily: "'Vazirmatn', sans-serif",
        }}>
      ...
      <Box onClick={handleImageClick} sx={{
        width: { xs: 100, sm: 140 }, minWidth: { xs: 100, sm: 140 },
        bgcolor: C.oliveSubtle, display: "flex",
        ...
        {imageUrl ? (
          <img src={imageUrl} alt={food.name} style={{
      ...
      {/* محتوا */}
      <Box sx={{
        flex: 1, px: { xs: 1.2, sm: 2 }, py: { xs: 1, sm: 1.5 },
        display: "flex", flexDirection: "column",
        justifyContent: "space-between", gap: { xs: 0.5, sm: 1 },
        minWidth: 0, overflow: "hidden",
      }}>
        {/* اسم */}
        <Box sx={{ minWidth: 0 }}>
          <Typography sx={{
            fontWeight: 800, fontSize: { xs: 13, sm: 16 }, color: C.text,
            fontFamily: "'Vazirmatn', sans-serif", lineHeight: 1.5,
          ...
          {showSubName && (
            <Typography sx={{
              fontSize: { xs: 10, sm: 12 }, color: C.muted, fontWeight: 400,
              whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis",
            }}>
              {food.name_en}
            </Typography>
        ...
        {editMode ? (
          <Box onClick={e => e.stopPropagation()} sx={{
            ...
            {/* قیمت */}
            <Box>
              <Typography sx={labelSx}>{isRtl ? "قیمت محصول" : "Price"}</Typography>
              <input
                type="text"
            ...
            <Box>
              <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 0.3 }}>
                <Typography sx={{ ...labelSx, mb: 0 }}>{isRtl ? "تخفیف" : "Discount"}</Typography>
                <Box sx={{ display: "flex", gap: 0.4 }}>
                  <IconButton size="small" onClick={() => { setDiscountMode("percent"); setDiscount(0); }} sx={toggleSx(discountMode === "percent")}>
                    <Percent sx={{ fontSize: 14 }} />
                  </IconButton>
                  <IconButton size="small" onClick={() => { setDiscountMode("amount"); setDiscount(0); }} sx={toggleSx(discountMode === "amount")}>
              ...
              {discount > 0 && (
                <Typography sx={{
                  fontSize: 11, color: C.burgundy, fontWeight: 700, mt: 0.4,
                  fontFamily: "'Vazirmatn', sans-serif",
                }}>
                  {discountMode === "percent"
            ...
            {/* موجودی */}
            <Box>
              <Typography sx={labelSx}>{isRtl ? "موجودی" : "Stock"}</Typography>
              <Box sx={{ display: "flex", gap: 0.6, alignItems: "center" }}>
                ...
                <Box
                  ...
                  sx={{
                    height: 34, px: 1.2, borderRadius: 8,
                    bgcolor: `${C.olive}12`, color: C.olive,
                    display: "flex", alignItems: "center", justifyContent: "center",
                    fontSize: 12, fontWeight: 800,
                    fontFamily: "'Vazirmatn', sans-serif",
                    cursor: "pointer", border: `1.5px solid ${C.olive}25`,
                    transition: "all 0.2s ease",
              ...
              {stock <= 0 && (
                <Typography sx={{ fontSize: 11, color: C.danger, fontWeight: 700, mt: 0.4, fontFamily: "'Vazirmatn', sans-serif" }}>
                  {isRtl ? "⚠ ناموجود" : "⚠ Out of stock"}
                </Typography>
              )}
              {lowStock && stock > 0 && (
                <Typography sx={{ fontSize: 11, color: C.gold, fontWeight: 700, mt: 0.4, fontFamily: "'Vazirmatn', sans-serif" }}>
            ...
            <Box
              ...
              sx={{
                ...
                bgcolor: saving ? `${C.olive}40` : C.olive,
                color: "#fff", display: "flex",
                alignItems: "center", justifyContent: "center",
                gap: 0.8, cursor: saving ? "wait" : "pointer",
          ...
          /* حالت نمایش */
          <Box sx={{ display: "flex", flexDirection: "column", gap: { xs: 0.4, sm: 0.8 } }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.6 }}>
              <Box sx={{
                width: { xs: 6, sm: 7 }, height: { xs: 6, sm: 7 },
                borderRadius: "50%",
            ...
            <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 0.5 }}>
              <Box sx={{ display: "flex", alignItems: "baseline", gap: 0.6, minWidth: 0, overflow: "hidden" }}>
                {discountAmount > 0 && (
                  <Typography sx={{
                    fontSize: { xs: 10, sm: 12 }, color: C.muted,
                ...
                <Typography sx={{
                  ...
                  {effectivePrice > 0 && (
                    <Typography component="span" sx={{
                      fontSize: { xs: 10, sm: 11 }, fontWeight: 600, color: C.muted, mr: 0.4,
                      fontFamily: "'Vazirmatn', sans-serif",
                    }}>
                      {isRtl ? "تومان" : "T"}
                    </Typography>
              ...
              {qty > 0 ? (
                <Box sx={{
                  ...
                  gap: { xs: 0.2, sm: 0.4 },
                  bgcolor: `${C.olive}10`, borderRadius: "10px", px: { xs: 0.2, sm: 0.3 }, flexShrink: 0,
                }}>
                  <IconButton onClick={dec} size="small" sx={{
                    width: { xs: 26, sm: 30 }, height: { xs: 26, sm: 30 },
                    borderRadius: "8px", color: C.olive,
                    "&:hover": { bgcolor: `${C.olive}15` },
                  }}>
                    <Remove sx={{ fontSize: { xs: 14, sm: 16 } }} />
                  </IconButton>
                  <Typography sx={{
                    fontSize: { xs: 13, sm: 14 }, fontWeight: 800, color: C.olive,
                    minWidth: { xs: 16, sm: 20 }, textAlign: "center",
                    fontFamily: "'Vazirmatn', sans-serif",
                  }}>
                    {qty}
                  </Typography>
                  <IconButton onClick={inc} size="small" sx={{
                    width: { xs: 26, sm: 30 }, height: { xs: 26, sm: 30 },
                    borderRadius: "8px",
                    bgcolor: outOfStock ? C.muted : C.olive, color: "#fff",
                    "&:hover": { bgcolor: outOfStock ? C.muted : C.olive, opacity: 0.85 },
                  }}>
                ...
                <IconButton onClick={inc} size="small" sx={{
                  ...
                  flexShrink: 0,
                  "&:hover": { bgcolor: outOfStock ? `${C.muted}20` : `${C.olive}22` },
                }}>
