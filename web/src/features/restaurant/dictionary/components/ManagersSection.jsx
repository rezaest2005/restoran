...
export default function MenuSection({ C, isRtl, isDark, showToast }) {
  ...
  const [customCategories, setCustomCategories] = useState(() => {
    try { return JSON.parse(localStorage.getItem("food_categories") || "[]"); }
    catch { return []; }
  });
  ...
  // ★ لیست کامل دسته‌بندی‌ها = از غذاها + دستی
  const categories = useMemo(() => {
    const cats = new Set();
    foods.forEach(f => { if (f.category_name) cats.add(f.category_name); });
    customCategories.forEach(c => cats.add(c));
    return [...cats].sort();
  }, [foods, customCategories]);

  const filtered = useMemo(() => {
    let list = foods;
    if (filterCat !== "__all__") list = list.filter(f => f.category_name === filterCat);
    if (search.trim()) {
    ...
    return list;
  }, [foods, filterCat, search]);

  // ★ ذخیره دسته‌بندی‌ها در localStorage
  const persistCategories = useCallback((cats) => {
    setCustomCategories(cats);
    localStorage.setItem("food_categories", JSON.stringify(cats));
  }, []);
  ...
  const handleRemoveCategory = (catName) => {
    const hasFood = foods.some(f => f.category_name === catName);
    if (hasFood) {
      showToast("این دسته‌بندی غذا دارد، اول غذاها رو عوض کن", "warning");
      return;
    }
    const updated = customCategories.filter(c => c !== catName);
    persistCategories(updated);
    showToast(`دسته‌بندی «${catName}» حذف شد`, "success");
  };

  const handleSave = async (data) => {
    try {
      // ★ اگه دسته‌بندی جدید تایپ شده، ذخیره کن
      if (data.category_name && !categories.includes(data.category_name)) {
        const updated = [...customCategories, data.category_name];
        persistCategories(updated);
      }
      if (data.id) {
        ...
        <Box sx={{ display: "flex", gap: 1 }}>
          <Button
            ...
            sx={{
              ...
              textTransform: "none", px: 2, py: 1, color: C.olive,
              border: `1px solid ${C.olive}40`,
              bgcolor: C.oliveSubtle,
              transition: "all 0.25s ease",
              "&:hover": { bgcolor: C.oliveSubtle, border: `1px solid ${C.olive}80` },
            }}
          >
            📁 {showCatPanel ? "بستن" : "دسته‌بندی‌ها"}
          </Button>
          <Button
        ...
        <Box sx={{
          ...
          <Box sx={{ display: "flex", gap: 1, mb: 2 }}>
            ...
            <Button
              ...
              sx={{
                borderRadius: "12px", fontSize: 12, fontWeight: 700,
                textTransform: "none", px: 2, py: 0.5, color: "#fff",
                background: C.btnGrad,
          ...
          <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.8 }}>
            {categories.length === 0 ? (
              ...
              categories.map(cat => {
                const isManual = customCategories.includes(cat);
                const foodCount = foods.filter(f => f.category_name === cat).length;
                return (
                  <Chip
                    key={cat}
                    label={`${cat} (${foodCount})`}
                    size="small"
                    onDelete={isManual ? () => handleRemoveCategory(cat) : undefined}
                    sx={{
          ...
          {filtered.map((food, i) => (
            <Box key={food.id} sx={{
              ...
              <Box sx={{ minWidth: { xs: 80, sm: 120 }, textAlign: "center" }}>
                {food.category_name ? (
                  <Chip size="small" label={food.category_name} sx={{
                    height: { xs: 20, sm: 22 },
                    fontSize: { xs: 9, sm: 10 },
                    fontWeight: 600, borderRadius: "6px",
                    bgcolor: C.oliveSubtle, color: C.olive, maxWidth: 100,
                  }} />
                ) : (
                  <Typography sx={{ fontSize: 10, color: C.muted }}>-</Typography>
                )}
              </Box>
              <Box sx={{
                display: "flex", gap: 0.3, justifyContent: "center",
                minWidth: { xs: 56, sm: 70 }, flexShrink: 0,
              }}>
